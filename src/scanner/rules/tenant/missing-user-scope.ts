...
/**
 * Detects API calls that don't include X-User-ID header.
 *
 * flowstackFetch() is a safe wrapper that injects X-User-ID from credentials,
 ...
 */
export class MissingUserScopeRule extends BaseRule {
  readonly meta: RuleMeta = {
    id: 'tenant/missing-user-scope',
    name: 'Missing User Scope on API Call',
    description: 'API calls should include X-User-ID header for user-level data isolation',
    severity: 'high',
    category: 'tenant',
    fixable: true,
    engine: 'ast',
  ...
  scan(context: RuleContext): Finding[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Skip flowstackFetch() — the wrapper injects X-User-ID from credentials
      ...
      if (isFlowstackDotFetch || isRawFetchToApi) {
        ...
        if (!block.includes('X-User-ID') && !block.includes('x-user-id')) {
          findings.push(
            this.createFinding(context, {
              line: i + 1,
              message:
                'Flowstack API call missing X-User-ID header. User data isolation is not enforced.',
              snippet: line.trim(),
              fix: {
                description: 'Add X-User-ID header to API call',
                strategyId: 'inject-tenant-scope',
                data: { header: 'X-User-ID' },
              },
            }),
          );
        }
